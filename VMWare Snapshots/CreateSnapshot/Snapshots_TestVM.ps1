...
Import-Csv -Path PATH\Servers.csv |
    ForEach-Object -Process { Get-VM -Name $_.TestVM |
    New-Snapshot -Name "Patching TestVM $(get-date -f dd-MM)" -Description $(Get-VM -Name $_.TestVM) }


# Checking if snapshots were created and creating report from it
Import-Csv -Path PATH\Servers.csv | Where-Object { $_.TestVM -ne ''} |
    ForEach-Object -Process { Get-VM -Name $_.TestVM | 
    Get-Snapshot | where {$_.name -like "Patching TestVM*"} |
